...
```{r}
gc_centered <- transmute(viruses, gc_diff = gc - mean(gc, na.rm = TRUE))
gc_centered
```
...
```{r}
ggplot(gc_centered) +
  geom_histogram(aes(gc_diff), bins = 30) +
  geom_vline(xintercept = 0, linetype = "dashed")
```

```{r}
gc_centered <- gc_centered %>% 
  mutate(gc_std = gc_diff / sd(gc_diff, na.rm = TRUE))
```

```{r}
ggplot(gc_centered) +
  geom_histogram(aes(gc_std), bins = 30)
```
...
```{r}
y <- c(1, 2, 2, NA, 3, 4)
message("This is original vector:", y)
min_rank(y)
min_rank(desc(y))
```
...
You can also use group_by together with mutate()!
...
```{r}
arrange(filter(summarise(group_by(viruses, subgroup, group), size_kb = mean(size_kb, na.rm = TRUE)), str_detect(group, "dsDNA")), desc(size_kb))
```
