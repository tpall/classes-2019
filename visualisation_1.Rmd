...
In R first 2 argument are also mapped by position, so you can skip "arg="
```{r}
set.seed(111)
ggplot(mpg) +
  geom_point(aes(cyl, hwy), position = position_jitter(0.24))
```

```{r}
ggplot(mpg) +
  geom_point(aes(cyl, hwy), position = "jitter")
```
...
```{r}
ggplot(data = mpg) +
  geom_jitter(mapping = aes(x = cyl, y = hwy), width = 0.24)
```
...
1. Update code. Map _**alpha**_ aesthetic to class:
```{r}
ggplot(data = mpg) + 
...
1. use log function (NB! log is natural logarithm)
```{r}
ggplot(data = starwars) +
  geom_point(mapping = aes(x = height, y = log10(mass), size = birth_year))
```

2. convert y-axit to log scale
```{r}
ggplot(data = starwars) +
  geom_point(mapping = aes(x = height, y = mass, size = birth_year)) +
  scale_y_log10()
...
```{r}
ggplot(data = mpg) +
  geom_point(mapping = aes(x = displ, y = hwy), color = "blue", size = 10) # try HEX code "#0000ff" or rgb(0,0,1)
```
