...
ggplot(data = mpg) +
  ...
  labs(caption = "Note that in case of the 'group' aesthetic ggplot2 does not add a legend or distinguishing features to the geoms. ")
```
...
Here, we create displ~hwy scatterplot with loess smooth line fitted trough all point, additionally we want to map color of points to the class of cars (whereas geom_smooth still plots only one line):
```{r}
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) +
...
```{r}
subcompacts <- filter(mpg, class == "subcompact")
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) +
  geom_point(aes(color = class)) +
  geom_smooth(data = subcompacts)
```
...
`geom_col()` uses x and y aesthetics, like other "normal" geoms.
```{r}
ggplot(data = diamonds_summarised) +
...
ggplot(data = diamonds) + 
  stat_summary(mapping = aes(x = cut, y = depth), 
               fun.data = mean_sdl, fun.args = list(mult = 1), geom = "errorbar", width = 0.5) +
  stat_summary(aes(x = cut, y = depth), fun.y = mean, geom = "point")
```
...
- coord_flip() switches the x and y axes. This is useful if you want horizontal boxplots. It's also very useful for long labels: it's hard to get them to fit without overlapping on the x-axis.
